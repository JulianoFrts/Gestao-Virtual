...
export default function Dashboard() {
  ...
  const stats = [
    ...
    },
    {
      ...
      perm: "daily_reports.view",
    },
  ];

  const quickActions = [
    { label: "Registrar Ponto", path: "/time-clock", icon: Clock, perm: "time_records.manage" },
    { label: "Novo Relatório", path: "/daily-report", icon: FileText, perm: "daily_reports.create" },
    { label: "Gerenciar Equipes", path: "/teams", icon: Users, perm: "teams.manage" },
  ];

  const filteredQuickActions = quickActions.filter((action) => can(action.perm));
  const filteredStats = stats.filter((stat) => can(stat.perm));
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Progress Overview */}
      {(can("time_records.view") || can("daily_reports.view")) && (
