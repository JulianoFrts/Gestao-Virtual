...
export async function POST(request: NextRequest) {
    try {
        ...
        logger.info(`[bulk_update_users] Iniciando atualização de ${ids.length} usuários`, {
            performerId: admin.id,
            fields: Object.keys(data)
        });

        const results = await userService.bulkUpdateUsers(ids, validationResult.data, admin.id);
        ...
        return handleApiError(error, "src/app/api/v1/rpc/bulk_update_users/route.ts#POST");
    }
}
