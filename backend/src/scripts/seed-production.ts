...
async function main() {
  ...
  try {
      ...
      await (prisma as any).mapElementProductionProgress.deleteMany({});
      await (prisma as any).stageProgress.deleteMany({});
      await (prisma as any).workStage.deleteMany({});
      await (prisma as any).productionActivity.deleteMany({});
      await (prisma as any).productionCategory.deleteMany({});
  } catch (e) {
  ...
  for (const catData of categories) {
    ...
    for (const activityData of activities) {
      ...
      console.log(`   - Activity created: ${activityData.name}`);
    }
  }
