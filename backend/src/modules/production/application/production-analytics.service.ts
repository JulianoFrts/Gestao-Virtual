...
export class ProductionAnalyticsService {
    ...
    async getPhysicalProgressCurve(params: AnalyticsParams) {
        ...
        towers.forEach(t => {
            ...
            const progresses = activityId && activityId !== 'all'
                ? t.mapElementProductionProgress.filter(p => p.activityId === activityId)
                : t.mapElementProductionProgress;

            schedules.forEach(sched => {
                const status = progresses.find(p => p.activityId === sched.activityId);
        ...
        const end = endDate ? endOfDay(new Date(endDate)) : endOfMonth(max(allDates));

        // 4. Geração da Timeline
        let timeline: Date[];
        if (granularity === 'weekly') {
            timeline = eachWeekOfInterval({ start, end: addWeeks(end, 1) }, { weekStartsOn: 1 });
        } else {
    ...
    async getFinancialData(params: {
        ...
        towers.forEach(t => {
            categories.forEach(cat => {
                cat.productionActivities.forEach(act => {
                    ...
                    const status = t.mapElementProductionProgress?.find(s => s.activityId === act.id);
        ...
        const filterStart = startDate ? startOfDay(new Date(startDate)) : minDateFound;
        ...
        // 5. Geração da Curva
        let timeline: Date[];
        if (granularity === 'weekly') timeline = eachWeekOfInterval({ start: filterStart, end: filterEnd }, { weekStartsOn: 1 });
        else if (granularity === 'monthly') timeline = eachMonthOfInterval({ start: filterStart, end: filterEnd });
        else if (granularity === 'quarterly') timeline = eachQuarterOfInterval({ start: filterStart, end: filterEnd });
        else if (granularity === 'annual') timeline = eachYearOfInterval({ start: filterStart, end: filterEnd });
