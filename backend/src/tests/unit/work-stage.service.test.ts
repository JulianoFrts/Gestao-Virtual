...
import { WorkStageRepository, WorkStage } from "../../modules/work-stages/domain/work-stage.repository";
...
describe("WorkStageService", () => {
    ...
    describe("findAll", () => {
        ...
        it("should call repo.findAll with normalized params", async () => {
            const stages: WorkStage[] = [{ id: "1", name: "Stage 1", siteId: "site-1", projectId: "proj-1", productionActivityId: null }];
            mockRepo.findAll.mockResolvedValue(stages);

            const result = await service.findAll({ siteId: "site-1", projectId: "all" });
    ...
    describe("createStage", () => {
        it("should throw error if name is missing", async () => {
            await expect(service.createStage({ name: "", siteId: "site-1", displayOrder: 0 }))
                .rejects.toThrow("Name is required");
        });

        it("should throw error if both siteId and projectId are missing", async () => {
            await expect(service.createStage({ name: "Test", siteId: "", projectId: "", displayOrder: 0 }))
        ...
        it("should create stage correctly when valid", async () => {
            ...
            const created: WorkStage = { id: "new-id", ...dto, projectId: null, productionActivityId: null };
        ...
        it("should handle invalid activityId by setting it to null", async () => {
            const dto = { name: "Test", siteId: "site-1", displayOrder: 1, productionActivityId: "invalid-uuid" };
            const created: WorkStage = { id: "new-id", ...dto, productionActivityId: null, projectId: null };
            mockRepo.create.mockResolvedValue(created);
        ...
        it("should verify activity existence if activityId is valid UUID", async () => {
            ...
            const dto = { name: "Test", siteId: "site-1", displayOrder: 1, productionActivityId: validUuid };

            mockRepo.verifyActivityExists.mockResolvedValue(false);
            mockRepo.create.mockResolvedValue({ id: "id", ...dto, productionActivityId: null, projectId: null });
