...
/**
 * v96.9.6: Orion PG Adapter - Forensic Mode
 * Dump total de OIDs e tradução bruta sem filtros.
 */
export class OrionPgAdapter {
  readonly provider = 'postgres';
  readonly adapterName = 'orion-pg-adapter-v96.9.6';

  constructor(private pool: pg.Pool) {
    console.log(`[Adapter/v96.9.6] Bridge forensic iniciada.`);
  }
  ...
  private translateEnum(fieldName: string, val: any): any {
    ...
    if (typeof val === 'string') {
      ...
      if (trimmed.length === 1) {
        ...
        if (mapped) {
          console.log(`[Adapter/v96.9.6] 🔄 Auto-Tradução Universal: ${fieldName} ('${val}' -> '${mapped}')`);
          return mapped;
        }
      }
    }
  ...
  private serializeValue(val: any, oid: number, fieldName: string): any {
    ...
    // Interceptação Forense (v96.9.6)
    if (typeof val === 'string' && val.trim().length === 1) {
      console.log(`[Adapter/v96.9.6] 🛡️ INTERCEPT: [${fieldName}] Raw='${val}' OID=${oid}`);
    }

    // Tradução Universal
    const translated = this.translateEnum(fieldName, val);

    // Inspeção Profunda (v96.9.6)
    if (typeof translated === 'string' && (translated === 'S' || translated === 'A')) {
      console.log(`[Adapter/v96.9.6] 🔍 Result [${fieldName}]: Value='${translated}' OID=${oid}`);
    }

    // Diagnóstico de Alerta
    if (typeof translated === 'string' && translated.length === 1 && ['S', 'A', 'U'].includes(translated.toUpperCase())) {
      console.log(`[Adapter/v96.9.6] ⚠️ Alerta Crítico: Valor bruto escapou em '${fieldName}': '${translated}' (OID: ${oid})`);
    }
  ...
  async queryRaw(params: { sql: string; args: any[] }) {
    try {
      ...
      // Diagnóstico de Estrutura (v96.9.6)
      if (params.sql.toLowerCase().includes('auth_credentials') || params.sql.toLowerCase().includes('select')) {
        ...
        console.log(`[Adapter/v96.9.6] 📡 Query [${res.rowCount} rows]: ${fieldDesc}`);
        if (res.rows.length > 0) {
          console.log(`[Adapter/v96.9.6] 🧪 Sample Raw: ${JSON.stringify(res.rows[0]).substring(0, 150)}`);
        }
      }
      ...
      console.error(`❌ [Adapter/v96.9.6] Query Error:`, err.message);
  ...
  async executeRaw(params: { sql: string; args: any[] }) {
    try {
      ...
      console.error(`❌ [Adapter/v96.9.6] Execute Error:`, err.message);
      return { ok: false, error: err };
    }
  }
...
const buildPrismaWithFallback = (url: string) => {
  ...
  try {
    ...
    console.warn(`⚠️ [Prisma/v96.9.6] Falha Crítica. Usando Modo Nativo.`);
