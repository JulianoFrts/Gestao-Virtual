...
/**
 * v97.3: Orion PG Adapter - Forensic Mode
 * Dump total de OIDs e tradução bruta sem filtros.
 */
export class OrionPgAdapter {
  readonly provider = 'postgres';
  readonly adapterName = 'orion-pg-adapter-v97.3';

  constructor(private pool: pg.Pool) {
    console.log(`[Adapter/v97.3] Bridge forensic iniciada.`);
  }
  ...
  private translateEnum(fieldName: string, val: any): any {
    ...
    if (typeof val === 'string') {
      ...
      if (trimmed.length === 1) {
        ...
        if (mapped) {
          console.log(`[Adapter/v97.3] 🔄 Auto-Tradução Universal: ${fieldName} ('${val}' -> '${mapped}')`);
          return mapped;
        }
      }
    }
  ...
  private serializeValue(val: any, oid: number, fieldName: string): any {
    ...
    // Interceptação Forense (v97.3)
    if (typeof val === 'string' && val.trim().length === 1) {
      console.log(`[Adapter/v97.3] 🛡️ INTERCEPT: [${fieldName}] Raw='${val}' OID=${oid}`);
    }

    // Tradução Universal
    const translated = this.translateEnum(fieldName, val);

    // Inspeção Profunda (v97.3)
    if (typeof translated === 'string' && (translated === 'S' || translated === 'A')) {
      console.log(`[Adapter/v97.3] 🔍 Result [${fieldName}]: Value='${translated}' OID=${oid}`);
    }

    // Diagnóstico de Alerta
    if (typeof translated === 'string' && translated.length === 1 && ['S', 'A', 'U'].includes(translated.toUpperCase())) {
      console.log(`[Adapter/v97.3] ⚠️ Alerta Crítico: Valor bruto escapou em '${fieldName}': '${translated}' (OID: ${oid})`);
    }
  ...
  async queryRaw(params: { sql: string; args: any[] }) {
    try {
      ...
      // Diagnóstico de Estrutura (v97.3)
      if (params.sql.toLowerCase().includes('auth_credentials') || params.sql.toLowerCase().includes('select')) {
        ...
        console.log(`[Adapter/v97.3] 📡 Query [${res.rowCount} rows]: ${fieldDesc}`);
        if (res.rows.length > 0) {
          console.log(`[Adapter/v97.3] 🧪 Sample Raw: ${JSON.stringify(res.rows[0]).substring(0, 150)}`);
        }
      }
      ...
      console.error(`❌ [Adapter/v97.3] Query Error:`, err.message);
  ...
  async executeRaw(params: { sql: string; args: any[] }) {
    try {
      ...
      console.error(`❌ [Adapter/v97.3] Execute Error:`, err.message);
      return { ok: false, error: err };
    }
  }
...
const buildPrismaWithFallback = (url: string) => {
  ...
  try {
    ...
    console.warn(`⚠️ [Prisma/v97.3] Falha Crítica. Usando Modo Nativo.`);
...
export const prisma = new Proxy({} as any, {
  get: (target, prop) => {
    ...
    try {
      ...
      console.error(`❌ [Prisma/v97.3] Proxy Error '${p}':`, err.message);
      return undefined;
    }
  }
...
const getSSLConfig = (connectionString: string) => {
  ...
  if (connectionString.includes('sslmode')) {
    ...
    if (cert && key) {
      ...
      console.log('🛡️ [Prisma/v97.3] mTLS v97.3 Ativo.');
    }
  }
  return sslConfig;
}
