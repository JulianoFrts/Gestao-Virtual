...
async function loadDependencies(): Promise<Dependencies> {
  const { ImportService } = await import(
    "@/modules/users/application/import.service"
  );
  ...
  const handlers: HandlerRegistration[] = [
    {
      ...
      loader: async () => {
        const { PermissionMatrixHandler } = await import(
          "@/modules/common/infrastructure/worker/handlers/permission-matrix.handler"
        );
    ...
    },
    {
      ...
      loader: async () => {
        const { JobFunctionImportHandler } = await import(
...
async function createWorker() {
  const { TaskWorker } = await import(
    "@/modules/common/application/task-worker"
  );
